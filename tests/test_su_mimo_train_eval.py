...
class SUMIMOTrainEvalCLITest(unittest.TestCase):
    ...
    def test_widely_linear_train_checkpoint_and_bler_eval(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            run_dir = Path(tmpdir) / "widely_linear"
            out_csv = Path(tmpdir) / "widely_linear_bler.csv"
            profile = (
                self.repo_root
                / "configs/channel_profiles/tdl_a_10_30_100_mix_normalized.json"
            )
            ...
            self.assertIn("model: su_mimo_widely_linear", trained.stdout)
            checkpoint = torch.load(
                run_dir / "best.pt", map_location="cpu", weights_only=True
            )
            self.assertEqual(
                checkpoint["model_name"], "su_mimo_widely_linear"
            )
            self.assertGreater(
                checkpoint["resolved_model_config"][
                    "widely_linear_hidden_channels"
                ],
                0,
            )
            ...
            with out_csv.open(newline="") as stream:
                rows = list(csv.DictReader(stream))
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0]["model"], "su_mimo_widely_linear")
